...
class TestHttpGetCiValidatorMetadataV1:
    ...
    def test_retrieve_all_ci_validator_metadata(self, setup_payload):
        ...
        make_iap_request("POST", f"{self.post_url}?{data[0]}",
                         json=setup_payload)  # Post CI with v2 endpoint, validator version = v0.0.1
        data = create_post_params(1, "0.0.2")
        make_iap_request("POST", f"{self.post_url}?{data[0]}",
                         json=setup_payload)  # Post CI with v2 endpoint, validator version = v0.0.2
