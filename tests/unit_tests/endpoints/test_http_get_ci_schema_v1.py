...
@patch(
    "app.repositories.buckets.ci_schema_bucket_repository.CiSchemaBucketRepository.retrieve_ci_schema"
)
class TestHttpGetCiSchemaV1:
    ...
    query_params = GetCiSchemaV1Params(
        form_type=mock_form_type, language=mock_language, survey_id=mock_survey_id
    )
    url = f"{base_url}?{urlencode(query_params.__dict__)}"

    def test_endpoint_returns_200_if_ci_schema_found(
    ...
    def test_endpoint_returns_BadRequest_if_metadata_not_found(
        ...
        expected_response = BadRequest(
            message=f"No metadata found for: {self.query_params.__dict__}"
        )
        response = client.get(self.url)
    ...
    def test_endpoint_returns_BadRequest_if_schema_not_found(
        self, mocked_retrieve_ci_schema, mocked_get_latest_ci_metadata
    ):
        """
        ...
        expected_response = BadRequest(
            message=f"No schema found for: {self.query_params.__dict__}"
        )
        response = client.get(self.url)
