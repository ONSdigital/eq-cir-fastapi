...
from tests.test_data.ci_test_data import (
    ...
    mock_post_ci_schema,
    mock_next_version_ci_metadata_v2,
    mock_ci_metadata_v2,
)
...
class TestHttpPostCiV2:
    """
    Tests for the `http_post_ci_v2` endpoint
    """

    url = "/v2/publish_collection_instrument"
    ...
    def test_endpoint_returns_200_if_ci_next_version_created_successfully(
        ...
        mocked_get_latest_ci_metadata.return_value = mock_ci_metadata_v2
        # Update mocked function to return a valid guid
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == mock_next_version_ci_metadata_v2.model_dump()
        CiFirebaseRepository.get_latest_ci_metadata.assert_called_once_with(
            mock_post_ci_schema.survey_id,
            mock_classifier_type,
            mock_classifier_value,
